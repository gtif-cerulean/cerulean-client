...
2\. For the demonstration version of the CIF Dashboard, the Landslide Risk capability is restricted to one location in Ireland. Using the two "Select date" boxes choose the dates to be compared for the analysis. Then click on the "Execute" button.

![](assets/landslide_risk_tutorial/ascreenshot_bee971377b144a529a77e5a743d518cd_text_export.jpeg)


3\. After a period of processing, the results are displayed on the map.

Since it takes some time for the results to be processed, they are stored for future access. For each stored run there is information on the date of execution and whether the run was successful or failed. There are options to load the results on the map, download the results, or delete the run.

![](assets/landslide_risk_tutorial/ascreenshot_20baeb5016634252bb019bb9652f3418_text_export.jpeg)
